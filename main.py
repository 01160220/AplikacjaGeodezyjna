...
class MyApp(QDialog):
    def __init__(self):
        ...
        self.ui.tabWidget_2.currentChanged.connect(self.zmiana_karty)
        self.ui.tabWidget.currentChanged.connect(self.zmiana_karty)
    ...
    def wybor_elipsoidy_SMS(self):
        if self.ui.radioButton_10.isChecked():
            self.a = 6378137.0
            self.b = 6356752.31414036
        elif self.ui.wgs_3.isChecked():
            self.a = 6378137.0 
            self.b = 6356752.31424518 
        else:
            msg = QMessageBox()
    ...
    def wprowadzanie_danych(self):
        if len(self.ui.fi_wprow_2.text()) != 0 and float(self.ui.fi_wprow_2.text()) >= 49 and float(self.ui.fi_wprow_2.text()) <= 54.8333: # zakres na phi <49 stopni, 54 stopni 50 minut>
            self.fi = float(self.ui.fi_wprow_2.text())
            ...
            msg.setWindowTitle("Sprawdź wprowadzone dane!")
            msg.exec_()
        if len(self.ui.lambda_wprow_2.text()) != 0 and float(self.ui.lambda_wprow_2.text()) >= 14.1167 and float(self.ui.lambda_wprow_2.text()) <= 24.15: # zakres na lambda <14 stopni 07 minut, 24 stopnie 09 minut>
            self.lam = float(self.ui.lambda_wprow_2.text())
    ...
    def XYZ(self):
        if len(self.ui.lineEdit.text()) != 0:
            self.hel = float(self.ui.lineEdit.text())
        else:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
            msg.setWindowIcon(QtGui.QIcon('wykrzyknik.png'))
